...
function createMsg(msg) {
    ...
    for (var i = 0; i < msg.length; i++) {
        // var imgSrc = msg[i].School_logo ? changeUrl.address.substring(0,13)+msg[i].School_logo : '../assets/img/schoolNoPic.png'; 
        var  imgSrc;
...
function instutionSearch() {
    ...
    $.ajax({
        ...
        url: changeUrl.address + "/Institution_api?whereFrom=count", //获取搜索的总条数
                ...
                laypage({
                    ...
                    jump: function(obj, first) {
                        ...
                        if (data.countAllRS != 0) {
                            myAjax(data01, changeUrl.address + "/Institution_api?whereFrom=search", createMsg)
                        } else {
                            $('#loadgif').hide()
                            $('#floatLayer').hide() //遮罩层
                        }
...
function accessData(fn) {
    ...
    if (jQuery.isEmptyObject(args)) {
        ...
        myAjax(data02, changeUrl.address + '/Institution_api?whereFrom=search', fn)
        ...
        $('#innerWrap').html('')
        myAjax(data01, changeUrl.address + '/Institution_api?whereFrom=search', fn)
    }
}
$(function() {
