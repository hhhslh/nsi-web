...
$(function() {
    $.ajax({
        ...
        success: function(msg) {
            console.log(msg.data[1].schoolLogo)
            for (var i = 0; i < msg.data.length; i++) {
                var imgSrc = msg.data[i].schoolLogo ? changeUrl.imgAddress + msg.data[i].schoolLogo : '../assets/img/schoolNoPic.png';
                $('#adviceSchoolWrap').append(
                    ...
                    ' <p> <span class="glyphicon glyphicon-time" style="top:2px;color:#215089;"></span> <span> 提交时间：</span> <span>' + msg.data[i].loadTime + '</span></p>' +
                    '</a>' +
                    '</div>'
                )
            }
